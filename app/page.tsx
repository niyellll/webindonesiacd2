...
export default function Page() {
  ...
  return (
    <main className="relative z-10 pt-24">
      {/* HERO (FIX: background layer must be z-0, not -z-10) */}
      <section className="relative min-h-[90vh] overflow-hidden px-6 py-20">
        <div aria-hidden="true" className="absolute inset-0 z-0">
          {/* glows */}
          <div className="absolute -top-48 left-1/2 h-[560px] w-[1100px] -translate-x-1/2 rounded-full blur-3xl
                          bg-gradient-to-r from-red-500/18 via-purple-500/10 to-blue-500/14
                          dark:from-red-500/22 dark:via-purple-500/14 dark:to-blue-500/18" />
          <div className="absolute -bottom-72 left-1/2 h-[520px] w-[980px] -translate-x-1/2 rounded-full blur-3xl
                          bg-gradient-to-r from-blue-500/12 via-red-500/10 to-transparent
                          dark:from-blue-500/18 dark:via-red-500/14" />
          {/* overlay to keep text readable in both modes */}
          <div className="absolute inset-0 bg-gradient-to-b from-white/35 via-white/10 to-white/35 dark:from-black/55 dark:via-black/25 dark:to-black/55" />
        </div>
        ...
        <div className="relative z-10 mx-auto flex max-w-7xl flex-col items-center justify-center text-center">
          <div className="mb-10 flex flex-wrap justify-center gap-3">
            {hero.chips.map((tag) => (
              <span
                key={tag}
                className="rounded-full border border-slate-300/70 bg-white/55 px-4 py-1.5 text-sm font-semibold text-slate-700 backdrop-blur
          ...
          <div className="flex flex-col gap-4 sm:flex-row">
            ...
            <a
              ...
              className="flex items-center justify-center gap-2 rounded-2xl border border-slate-200/70 bg-white/55 px-10 py-5 text-lg font-bold text-slate-900 backdrop-blur transition hover:bg-white/80
          ...
          {partners.cards.map((cat) => (
            <GlassCard key={cat.title} className="p-10">
              ...
              <div className="mb-4 flex items-center gap-4">
                ...
                <h3 className="text-2xl font-bold tracking-tight text-slate-900 dark:text-white">{cat.title}</h3>
              </div>
              <p className="mb-8 text-slate-600 dark:text-slate-300">{cat.desc}</p>
              <a href={`mailto:${site.email}`} className="inline-flex items-center gap-2 font-bold text-blue-600 hover:text-blue-700 dark:text-blue-400 dark:hover:text-blue-300">
      ...
      <section id="get-involved" className="mx-auto max-w-7xl px-6 py-28">
        ...
        <div className="mb-12 grid gap-7 lg:grid-cols-3">
          {getInvolved.cards.map((card) => (
            <GlassCard key={card.title} className="p-10 transition hover:-translate-y-1.5">
              <div className="mb-8 h-1.5 w-16 rounded-full bg-gradient-to-r from-red-600 to-blue-600 opacity-80" />
              <div className="mb-6 flex items-center gap-4">
                <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-slate-100 dark:bg-slate-800/70 text-slate-900 dark:text-white">
                ...
                <div>
                  <div className="text-2xl font-bold tracking-tight text-slate-900 dark:text-white">{card.title}</div>
        ...
        <div className="relative overflow-hidden rounded-[44px] bg-gradient-to-br from-red-600 to-blue-700 p-12 text-white shadow-[0_30px_120px_rgba(37,99,235,0.22)] md:p-20">
          ...
          <div className="relative z-10 text-center">
            ...
            <div className="flex flex-col justify-center gap-5 sm:flex-row">
              <a href={getInvolved.banner.primary.href} className="rounded-2xl bg-white px-12 py-5 text-xl font-black text-slate-900 transition hover:scale-[1.03]">
                {getInvolved.banner.primary.label}
              </a>
              <a href={getInvolved.banner.secondary.href} className="rounded-2xl border-2 border-white/80 bg-white/10 px-12 py-5 text-xl font-black text-white backdrop-blur transition hover:bg-white/15">
